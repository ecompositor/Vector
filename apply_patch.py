#!/usr/bin/env python
"""
    Patches Vector SDK with feature to avoid taking control on connect.
    Copyright (c) 2018

    Based on Patch utility to apply unified diffs
...
# -------------------------- The Patch data for applying to the SDK -----------------------------------
patch_data = br'''--- connection.py	Sun Oct 21 18:37:10 2018
...
def main():
  print("Anki Vector SDK patch tool")
  print()
  print("This tool patches the SDK package to include additional functionality.")
  print()
  ...
  for path in sys.path:
    ...
    if os.path.abspath(os.path.dirname(__file__)) == path: continue
    sdk_path = os.path.join(path, 'anki_vector')
  ...
  if not options.directory:
    options.directory = sdk_path

  if not options.directory:  
    opt.print_help()
    sys.exit()

  print(f"Patching the Path: {options.directory}")
  print()
  ...
  if options.revert:
    patch.revert(0, root=options.directory) or sys.exit(-1)
    os.remove(os.path.join(options.directory, "patched.txt"))
    print("Reverted.")
  else:
    with open(os.path.join(options.directory, "patched.txt"), "w") as text_file: print("1", file=text_file)
    patch.apply(0, root=options.directory) or sys.exit(-1)
    print("Done.")
